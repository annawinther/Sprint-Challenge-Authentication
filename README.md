...
Sessions are commonly used to allow server to store information about a client which then can be used for a variety of purposes. Most commonly its used to persist authentication information so there is no need to re enter credentials on every new request the client makes to the server. They provide a way to resist the data across reqs.

1. What does bcrypt do to help us store passwords in a secure manner.

Bcrypt has a password hashing function, it implements salting both manually and automatically and it has accumulative hashing rounds. We can also use bcrypt to verify passwords by using compareSync which takes two args, one is the password guess in lain text and the other one is the password hash obtained from the database to validate the credentials.

1. What does bcrypt do to slow down attackers?

It has the hashing function and the hashing round which hashes the information multiple times (rounds).This means that an attacker needs to have the hash, know the algorithm used and know exactly how many rounds were used to generate the hash in the first place.

1. What are the three parts of the JSON Web Token?

Header -  which contain the algorithm and the token type
Payload - which contains information about the actual data
Signature - which is the whole thing. To create a signature we need to create a sting by base64,which encodes the payload and header together and then signing it with a secret
